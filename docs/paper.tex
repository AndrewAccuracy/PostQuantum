...
的时间侧信道泄漏。该结论属于实现级筛查结论，不构成形式化侧信道安全证明。
...
Mechanism，即 ML-KEM~\cite{FIPS203,Kyber2018}。ML-KEM 基于模格误差学习
...
操作系统之上，因此同样需要关注输入相关的执行时间差异。因此，本文将执行时间作为
可观测信道，关注攻击者可控输入是否会在 ML-KEM 解封装实现中诱发稳定的时间差异。
...
本文面向小样本、白盒、筛查型的软件计时任务，因此选择逻辑回归、线性 SVM、随机
森林和直方图梯度提升树作为检测模型。这些模型既能覆盖线性与非线性判别，又便于与
统计检验、置换检验和特征重要性分析结合。本文将机器学习分类结果作为统计检验之外的
补充证据，而不是以模型准确率替代传统泄漏评估。
...
集成树模型~\cite{Breiman2001}，梯度提升树参考提升树的逐步残差拟合思想~\cite{Chen2016}，
...
具备选择密文能力：可以构造任意同长度密文并提交给目标实现的解封装接口，但不能
访问私钥或其内部状态，目标与 CCA 安全模型中的攻击者一致。攻击者通过被测进程自身
提供的高精度计时接口（Python \texttt{time.perf\_counter\_ns}）获取每次解封装调用
的墙钟时间，不依赖缓存探测、性能计数器或功耗/电磁探针等更强的旁路观测手段。
...
每条聚合轨迹重复执行50次解封装，并记录纳秒级原始时间。整体流程如算法~\ref{alg:collect}
所示。
...
均未触发泄漏判定；Welch's $t$ 检验5轮均值均远高于0.01；KS 检验均值也未形成稳定
显著模式；Cohen's $d$ 的组合均值绝对值最高仅为0.0448，低于可忽略效应阈值0.2。

由于本文在12个参数集/策略组合上重复进行假设检验，单一组合的 $p<0.01$ 判定还需
考虑多重比较带来的族系错误率（family-wise error rate）膨胀问题。按 Holm-Bonferroni
方法，在族系显著性水平0.01、共12次比较的条件下，最严格的单次比较阈值约为
$0.01/12\approx8.3\times10^{-4}$。表~\ref{tab:matrix}中最小的 Welch's $t$ 检验
均值为0.262（1024、\texttt{zero}），仍比该阈值大两个数量级以上。因此，即使不做
多重比较校正，本文也未在任一组合上观察到接近显著的结果；引入校正只会使各组合
更难被判定为显著，不改变"未检测到泄漏"的结论，但能排除多重检验导致的假阳性
风险。
...
为将上述经验观察转化为可量化的检测下界，本文基于两样本 $t$ 检验的功效近似公式
估计本文实验设计下的最小可检测效应量~\cite{Cohen1988}：
\begin{equation}
...
表~\ref{tab:mde}中80\%功效对应的最小可检测效应（2.4--3.5~$\mu$s，视参数集而定）
...
因此，本文真实场景"未检测到泄漏"的结论可更精确地表述为：在每类 $n=400$、
$\alpha=0.01$ 的实验设计下，本文能以约80\%的把握排除大于2.4--3.5~$\mu$s（视参数
集而定）的稳定时间差异；量级更小的潜在差异不在本文方法的可分辨范围内。
...
命令见附录。第三方可在安装相同版本依赖后，直接基于附录中的命令重新生成本文表
~\ref{tab:matrix}--\ref{tab:sweep}和图~\ref{fig:all_dist}--\ref{fig:stability}
中的全部结果。
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    路径 & 说明 \\
    \midrule
    \texttt{results/paper\_runs/} & 5轮完整实验的原始 CSV 与 JSON 分析结果 \\
    \texttt{results/paper\_artifacts/} & 论文图表与数据质量报告 \\
    \texttt{DATA\_QUALITY\_REPORT.md} & 标签平衡、缺失值、正对照有效性等检查结果 \\
    \texttt{summary.json} & 每个参数集/策略组合的统计检验和模型结果 \\
    \bottomrule
  \end{tabular}
\end{table}
